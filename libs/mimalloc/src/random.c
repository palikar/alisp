...
The implementation uses regular C code which compiles very well on modern
compilers. (gcc x64 has no register spills, and clang 6+ uses SSE instructions)
-----------------------------------------------------------------------------*/
...
static inline void qround(uint32_t x[16], size_t a, size_t b, size_t c,
                          size_t d) {
  x[a] += x[b];
  x[d] = rotl(x[d] ^ x[a], 16);
  x[c] += x[d];
...
static uint32_t chacha_next32(mi_random_ctx_t* ctx) {
  ...
  ctx->output[16 - ctx->output_available] =
      0;  // reset once the data is handed out
  ctx->output_available--;
  return x;
}
...
static inline uint32_t read32(const uint8_t* p, size_t idx32) {
  ...
  return ((uint32_t)p[i + 0] | (uint32_t)p[i + 1] << 8 |
          (uint32_t)p[i + 2] << 16 | (uint32_t)p[i + 3] << 24);
}

static void chacha_init(mi_random_ctx_t* ctx, const uint8_t key[32],
                        uint64_t nonce) {
...
static void chacha_split(mi_random_ctx_t* ctx, uint64_t nonce,
  ...
  mi_assert_internal(ctx->input[14] != ctx_new->input[14] ||
                     ctx->input[15] !=
                         ctx_new->input[15]);  // do not reuse nonces!
...
If we cannot get good randomness, we fall back to weak randomness based on a
timer and ASLR.
-----------------------------------------------------------------------------*/
...
*/
#elif defined(ANDROID) || defined(XP_DARWIN) || defined(__APPLE__) ||        \
    defined(__DragonFly__) || defined(__FreeBSD__) || defined(__NetBSD__) || \
    defined(__OpenBSD__) || defined(__wasi__)
#include <stdlib.h>
static bool os_random_buf(void* buf, size_t buf_len) {
...
static bool os_random_buf(void* buf, size_t buf_len) {
  ...
  // `sys/random.h` or `linux/random.h` and for the latter the actual
  // `getrandom` call is not always defined. (see
  // <https://stackoverflow.com/questions/45237324/why-doesnt-getrandom-compile>)
  // We therefore use a syscall directly and fall back dynamically to
  // /dev/urandom when needed.
#ifdef SYS_getrandom
...
#define GRND_NONBLOCK (1)
  ...
  if (mi_atomic_read(&no_getrandom) == 0) {
    ...
    mi_atomic_write(&no_getrandom,
                    1);  // don't call again, and fall back to /dev/urandom
  }
#endif
  int flags = O_RDONLY;
...
#else
static bool os_random_buf(void* buf, size_t buf_len) { return false; }
#endif
...
uintptr_t _os_random_weak(uintptr_t extra_seed) {
  uintptr_t x = (uintptr_t)&_os_random_weak ^
                extra_seed;  // ASLR makes the address random
#if defined(_WIN32)
