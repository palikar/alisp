...
static void mi_stat_update(mi_stat_count_t* stat, int64_t amount) {
  ...
  if (mi_is_in_main(stat)) {
    ...
    if (stat->current > stat->peak)
      stat->peak = stat->current;  // racing.. it's ok
    if (amount > 0) {
...
static void mi_printf_amount(int64_t n, int64_t unit, mi_output_fun* out,
...
static void mi_stat_counter_print(const mi_stat_counter_t* stat,
                                  const char* msg, mi_output_fun* out,
                                  void* arg) {
  _mi_fprintf(out, arg, "%10s:", msg);
...
static void mi_stat_counter_print_avg(const mi_stat_counter_t* stat,
                                      const char* msg, mi_output_fun* out,
                                      void* arg) {
  const int64_t avg_tens =
      (stat->count == 0 ? 0 : (stat->total * 10 / stat->count));
...
#if MI_STAT > 1
static void mi_stats_print_bins(mi_stat_count_t* all,
                                const mi_stat_count_t* bins, size_t max,
                                const char* fmt, mi_output_fun* out,
                                void* arg) {
  bool found = false;
  char buf[64];
...
static void mi_process_info(mi_msecs_t* utime, mi_msecs_t* stime,
                            size_t* peak_rss, size_t* page_faults,
                            size_t* page_reclaim, size_t* peak_commit);

static void _mi_stats_print(mi_stats_t* stats, mi_msecs_t elapsed,
                            mi_output_fun* out0, void* arg0) mi_attr_noexcept {
  ...
  mi_stat_print(&stats->huge, "huge",
                ...
                out, arg);
  mi_stat_print(&stats->giant, "giant",
                (stats->giant_count.count == 0
                     ? 1
                     : -(stats->giant.allocated / stats->giant_count.count)),
                out, arg);
  ...
  mi_process_info(&user_time, &sys_time, &peak_rss, &page_faults, &page_reclaim,
                  &peak_commit);
  _mi_fprintf(out, arg,
              "%10s: user: %ld.%03ld s, system: %ld.%03ld s, faults: %lu, "
              "reclaims: %lu, rss: ",
              "process", user_time / 1000, user_time % 1000, sys_time / 1000,
...
void mi_stats_print(void* out) mi_attr_noexcept {
  // for compatibility there is an `out` parameter (which can be `stdout` or
  // `stderr`)
