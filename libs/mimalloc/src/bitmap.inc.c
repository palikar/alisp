...
static inline size_t mi_bitmap_index_bit_in_field(
...
/* -----------------------------------------------------------
  Use bit scan forward/reverse to quickly find the first zero bit if it is
available
----------------------------------------------------------- */
#if defined(_MSC_VER)
...
static inline size_t mi_bsr(uintptr_t x) {
  return (x == 0 ? 8 * MI_INTPTR_SIZE
                 : (8 * MI_INTPTR_SIZE - 1) - MI_L(__builtin_clz)(x));
}
#endif
...
static inline bool mi_bitmap_try_find_claim_field(
    mi_bitmap_t bitmap, size_t idx, const size_t count,
    mi_bitmap_index_t* bitmap_idx) {
  ...
  while (bitidx <= bitidx_max) {
    if ((map & m) == 0) {  // are the mask bits free at bitidx?
      ...
      if (!mi_atomic_cas_weak(field, newmap,
                              map)) {  // TODO: use strong cas here?
...
// Find `count` bits of 0 and set them to 1 atomically; returns `true` on
// success. For now, `count` can be at most MI_BITMAP_FIELD_BITS and will never
// span fields.
static inline bool mi_bitmap_try_find_claim(mi_bitmap_t bitmap,
                                            size_t bitmap_fields, size_t count,
                                            mi_bitmap_index_t* bitmap_idx) {
...
static inline bool mi_bitmap_unclaim(mi_bitmap_t bitmap, size_t bitmap_fields,
                                     size_t count,
                                     mi_bitmap_index_t bitmap_idx) {
  const size_t idx = mi_bitmap_index_field(bitmap_idx);
  const size_t bitidx = mi_bitmap_index_bit_in_field(bitmap_idx);
...
static inline bool mi_bitmap_claim(mi_bitmap_t bitmap, size_t bitmap_fields,
...
// Returns `true` if all `count` bits were 1. `any_ones` is `true` if there was
// at least one bit set to one.
static inline bool mi_bitmap_is_claimedx(mi_bitmap_t bitmap,
                                         size_t bitmap_fields, size_t count,
                                         mi_bitmap_index_t bitmap_idx,
                                         bool* any_ones) {
  const size_t idx = mi_bitmap_index_field(bitmap_idx);
...
static inline bool mi_bitmap_is_claimed(mi_bitmap_t bitmap,
                                        size_t bitmap_fields, size_t count,
                                        mi_bitmap_index_t bitmap_idx) {
  return mi_bitmap_is_claimedx(bitmap, bitmap_fields, count, bitmap_idx, NULL);
}

static inline bool mi_bitmap_is_any_claimed(mi_bitmap_t bitmap,
                                            size_t bitmap_fields, size_t count,
                                            mi_bitmap_index_t bitmap_idx) {
  bool any_ones;
