...
static bool mi_heap_page_collect(mi_heap_t* heap, mi_page_queue_t* pq,
                                 mi_page_t* page, void* arg_collect,
                                 void* arg2) {
  UNUSED(arg2);
  UNUSED(heap);
...
static bool mi_heap_page_never_delayed_free(mi_heap_t* heap,
                                            mi_page_queue_t* pq,
                                            mi_page_t* page, void* arg1,
                                            void* arg2) {
  UNUSED(arg1);
  UNUSED(arg2);
  UNUSED(heap);
...
static void mi_heap_collect_ex(mi_heap_t* heap, mi_collect_t collect) {
  ...
  if (
      ...
      !heap->no_reclaim) {
    // the main thread is abandoned (end-of-program), try to reclaim all
    // abandoned segments. if all memory is freed by now, all segments should be
    // freed.
  ...
  // (if abandoning, after this there are no more thread-delayed references into
  // the pages.)
  ...
  mi_assert_internal(
      collect != MI_ABANDON ||
      mi_atomic_read_ptr(mi_block_t, &heap->thread_delayed_free) == NULL);
...
mi_heap_t* mi_heap_new(void) {
  ...
  heap->no_reclaim =
      true;  // don't reclaim abandoned pages or otherwise destroy is unsafe
...
static bool _mi_heap_page_destroy(mi_heap_t* heap, mi_page_queue_t* pq,
                                  mi_page_t* page, void* arg1, void* arg2) {
  UNUSED(arg1);
  UNUSED(arg2);
  UNUSED(heap);
  ...
  mi_heap_stat_decrease(heap, malloc,
                        bsize * inuse);  // todo: off for aligned blocks...
#endif
...
static void mi_heap_absorb(mi_heap_t* heap, mi_heap_t* from) {
  ...
  // note: appending waits for each page to not be in the `MI_DELAYED_FREEING`
  // state so after this only the new heap will get delayed frees
  for (size_t i = 0; i <= MI_BIN_FULL; i++) {
    mi_page_queue_t* pq = &heap->pages[i];
  ...
  // note: be careful here as the `heap` field in all those pages no longer
  // point to `from`, turns out to be ok as `_mi_heap_delayed_free` only visits
  // the list and calls a the regular `_mi_free_delayed_block` which is safe.
  _mi_heap_delayed_free(from);
...
#define MI_MAX_BLOCKS (MI_SMALL_PAGE_SIZE / sizeof(void*))
  ...
  for (mi_block_t* block = page->free; block != NULL;
    ...
    free_count++;
    mi_assert_internal((uint8_t*)block >= pstart &&
                       (uint8_t*)block < (pstart + psize));
...
typedef bool(mi_heap_area_visit_fun)(const mi_heap_t* heap,
                                     const mi_heap_area_ex_t* area, void* arg);
...
static bool mi_heap_visit_areas(const mi_heap_t* heap,
                                mi_heap_area_visit_fun* visitor, void* arg) {
...
static bool mi_heap_area_visitor(const mi_heap_t* heap,
                                 const mi_heap_area_ex_t* xarea, void* arg) {
  mi_visit_blocks_args_t* args = (mi_visit_blocks_args_t*)arg;
  if (!args->visitor(heap, &xarea->area, NULL, xarea->area.block_size,
                     args->arg))
    return false;
