...
static uint8_t* mi_segment_raw_page_start(const mi_segment_t* segment,
...
/* --------------------------------------------------------------------------------
  ...
  In any case the memory for a segment is virtual and usually committed on
demand. (i.e. we are careful to not touch the memory until we actually allocate
a block there)
...
#if (MI_DEBUG >= 3)
static bool mi_segment_queue_contains(const mi_segment_queue_t* queue,
...
static void mi_segment_queue_remove(mi_segment_queue_t* queue,
...
static void mi_segment_enqueue(mi_segment_queue_t* queue,
                               mi_segment_t* segment) {
  mi_assert_expensive(!mi_segment_queue_contains(queue, segment));
  segment->next = NULL;
...
static mi_segment_queue_t* mi_segment_free_queue_of_kind(
    mi_page_kind_t kind, mi_segments_tld_t* tld) {
...
static mi_segment_queue_t* mi_segment_free_queue(const mi_segment_t* segment,
                                                 mi_segments_tld_t* tld) {
  return mi_segment_free_queue_of_kind(segment->page_kind, tld);
}

// remove from free queue if it is in one
static void mi_segment_remove_from_free_queue(mi_segment_t* segment,
                                              mi_segments_tld_t* tld) {
  mi_segment_queue_t* queue =
      mi_segment_free_queue(segment, tld);  // may be NULL
  bool in_queue =
      (queue != NULL && (segment->next != NULL || segment->prev != NULL ||
                         queue->first == segment));
  if (in_queue) {
...
static void mi_segment_insert_in_free_queue(mi_segment_t* segment,
                                            mi_segments_tld_t* tld) {
...
#if (MI_DEBUG >= 2)
static bool mi_segment_is_in_free_queue(const mi_segment_t* segment,
                                        mi_segments_tld_t* tld) {
...
#if (MI_DEBUG >= 2)
static bool mi_pages_reset_contains(const mi_page_t* page,
                                    mi_segments_tld_t* tld) {
...
#if (MI_DEBUG >= 3)
static bool mi_segment_is_valid(const mi_segment_t* segment,
                                mi_segments_tld_t* tld) {
  mi_assert_internal(segment != NULL);
  ...
  // (segment->thread_id==0)); // or 0
  mi_assert_internal(segment->page_kind == MI_PAGE_HUGE ||
                     (mi_segment_page_size(segment) * segment->capacity ==
                      segment->segment_size));
  return true;
}
#endif

static bool mi_page_not_in_queue(const mi_page_t* page,
                                 mi_segments_tld_t* tld) {
...
static void mi_segment_protect(mi_segment_t* segment, bool protect,
  ...
  if (MI_SECURE != 0) {
    ...
    mi_assert_internal(
        (segment->segment_info_size - os_page_size) >=
        (sizeof(mi_segment_t) + ((segment->capacity - 1) * sizeof(mi_page_t))));
    mi_assert_internal(
        ((uintptr_t)segment + segment->segment_info_size) % os_page_size == 0);
    mi_segment_protect_range(
        (uint8_t*)segment + segment->segment_info_size - os_page_size,
        os_page_size, protect);
    if (MI_SECURE <= 1 || segment->capacity == 1) {
      ...
      for (size_t i = 0; i < segment->capacity; i++) {
        if (segment->pages[i].is_committed) {
          mi_segment_protect_range(
              (uint8_t*)segment + (i + 1) * page_size - os_page_size,
              os_page_size, protect);
        }
      }
    }
...
static void mi_page_reset(mi_segment_t* segment, mi_page_t* page, size_t size,
                          mi_segments_tld_t* tld) {
  mi_assert_internal(page->is_committed);
  if (!mi_option_is_enabled(mi_option_page_reset)) return;
  if (segment->mem_is_fixed || page->segment_in_use || !page->is_committed ||
      page->is_reset)
    return;
  size_t psize;
...
static void mi_page_unreset(mi_segment_t* segment, mi_page_t* page, size_t size,
                            mi_segments_tld_t* tld) {
...
static void mi_page_reset_set_expire(mi_page_t* page) {
  uint32_t expire =
      (uint32_t)_mi_clock_now() + mi_option_get(mi_option_reset_delay);
  page->used = expire;
}
...
static void mi_pages_reset_add(mi_segment_t* segment, mi_page_t* page,
                               mi_segments_tld_t* tld) {
...
static void mi_pages_reset_remove_all_in_segment(mi_segment_t* segment,
                                                 bool force_reset,
                                                 mi_segments_tld_t* tld) {
...
// Raw start of the page available memory; can be used on uninitialized pages
// (only `segment_idx` must be set) The raw start is not taking aligned block
// allocation into consideration.
static uint8_t* mi_segment_raw_page_start(const mi_segment_t* segment,
                                          const mi_page_t* page,
                                          size_t* page_size) {
  size_t psize =
      (segment->page_kind == MI_PAGE_HUGE ? segment->segment_size
                                          : (size_t)1 << segment->page_shift);
  ...
  if (MI_SECURE > 1 ||
      (MI_SECURE == 1 && page->segment_idx == segment->capacity - 1)) {
...
// Start of the page available memory; can be used on uninitialized pages (only
// `segment_idx` must be set)
uint8_t* _mi_segment_page_start(const mi_segment_t* segment,
                                const mi_page_t* page, size_t block_size,
                                size_t* page_size, size_t* pre_size) {
  size_t psize;
  uint8_t* p = mi_segment_raw_page_start(segment, page, &psize);
  if (pre_size != NULL) *pre_size = 0;
  if (page->segment_idx == 0 && block_size > 0 &&
      segment->page_kind <= MI_PAGE_MEDIUM) {
    // for small and medium objects, ensure the page start is aligned with the
    // block size (PR#66 by kickunderscore)
...
static size_t mi_segment_size(size_t capacity, size_t required,
                              size_t* pre_size, size_t* info_size) {
  const size_t minsize = sizeof(mi_segment_t) +
                         ((capacity - 1) * sizeof(mi_page_t)) +
                         16 /* padding */;
  size_t guardsize = 0;
  size_t isize = 0;
  ...
  return (required == 0 ? MI_SEGMENT_SIZE
                        : _mi_align_up(required + isize + 2 * guardsize,
                                       MI_PAGE_HUGE_ALIGN));
}
...
static void mi_segment_os_free(mi_segment_t* segment, size_t segment_size,
                               mi_segments_tld_t* tld) {
  segment->thread_id = 0;
  mi_segments_track_size(-((long)segment_size), tld);
  if (MI_SECURE != 0) {
    mi_assert_internal(!segment->mem_is_fixed);
    mi_segment_protect(segment, false,
                       tld->os);  // ensure no more guard pages are set
  }
  ...
  _mi_mem_free(segment, segment_size, segment->memid, fully_committed,
               any_reset, tld->os);
}

// The thread local segment cache is limited to be at most 1/8 of the peak size
// of segments in use,
#define MI_SEGMENT_CACHE_FRACTION (8)

// note: returned segment may be partially reset
static mi_segment_t* mi_segment_cache_pop(size_t segment_size,
                                          mi_segments_tld_t* tld) {
...
static bool mi_segment_cache_full(mi_segments_tld_t* tld) {
  // if (tld->count == 1 && tld->cache_count==0) return false; // always cache
  // at least the final segment of a thread
  size_t max_cache = mi_option_get(mi_option_segment_cache);
  if (tld->cache_count < max_cache &&
      tld->cache_count <
          (1 + (tld->peak_count /
                MI_SEGMENT_CACHE_FRACTION))  // at least allow a 1 element cache
  ) {
    return false;
  }
  // take the opportunity to reduce the segment cache if it is too large (now)
  // TODO: this never happens as we check against peak usage, should we use
  // current usage instead?
  while (tld->cache_count >
         max_cache) {  //(1 + (tld->peak_count / MI_SEGMENT_CACHE_FRACTION))) {
...
static bool mi_segment_cache_push(mi_segment_t* segment,
                                  mi_segments_tld_t* tld) {
...
static mi_segment_t* mi_segment_init(mi_segment_t* segment, size_t required,
  ...
  mi_assert_internal(segment == NULL ||
                     (required == 0 && page_kind <= MI_PAGE_LARGE));
  ...
  size_t pre_size;
  size_t segment_size =
      mi_segment_size(capacity, required, &pre_size, &info_size);
  mi_assert_internal(segment_size >= required);

  // Initialize parameters
  const bool eager_delayed =
      (page_kind <= MI_PAGE_MEDIUM &&
       tld->count < (size_t)mi_option_get(mi_option_eager_commit_delay));
  const bool eager =
      !eager_delayed && mi_option_is_enabled(mi_option_eager_commit);
  ...
  if (segment != NULL) {
    ...
    if (page_kind <= MI_PAGE_MEDIUM && segment->page_kind == page_kind &&
      ...
      if (MI_SECURE != 0) {
        ...
        mi_segment_protect(
            segment, false,
            tld->os);  // reset protection if the page kind differs
      }
      ...
      for (size_t i = 0; i < segment->capacity; i++) {
        ...
        if (page->is_reset) {
          if (!commit && mi_option_is_enabled(mi_option_reset_decommits)) {
            ...
            mi_page_unreset(segment, page, 0,
                            tld);  // todo: only unreset the part that was
                                   // reset? (instead of the full page)
          }
        }
      }
    ...
    bool mem_large =
        (!eager_delayed &&
         (MI_SECURE ==
          0));  // only allow large OS pages once we are no longer lazy
    segment = (mi_segment_t*)_mi_mem_alloc_aligned(
        segment_size, MI_SEGMENT_SIZE, &commit, &mem_large, &is_zero, &memid,
        os_tld);
    ...
    if (!commit) {
      ...
      if (commit_zero) is_zero = true;
      if (!ok) {
        // commit failed; we cannot touch the memory: free the segment directly
        // and return `NULL`
  ...
  mi_assert_internal(segment != NULL &&
                     (uintptr_t)segment % MI_SEGMENT_SIZE == 0);
...
static void mi_segment_free(mi_segment_t* segment, bool force,
                            mi_segments_tld_t* tld) {
  UNUSED(force);
  mi_assert(segment != NULL);
  // note: don't reset pages even on abandon as the whole segment is freed? (and
  // ready for reuse)
  bool force_reset =
      (force && mi_option_is_enabled(mi_option_abandoned_page_reset));
...
static bool mi_segment_page_claim(mi_segment_t* segment, mi_page_t* page,
                                  mi_segments_tld_t* tld) {
  mi_assert_internal(_mi_page_segment(page) == segment);
  mi_assert_internal(!page->segment_in_use);
  mi_pages_reset_remove(page, tld);
  ...
  if (page->is_reset) {
    mi_page_unreset(segment, page, 0,
                    tld);  // todo: only unreset the part that was reset?
  }
  mi_assert_internal(page->segment_in_use);
  mi_assert_internal(segment->used <= segment->capacity);
...
static void mi_segment_page_clear(mi_segment_t* segment, mi_page_t* page,
                                  bool allow_reset, mi_segments_tld_t* tld) {
  mi_assert_internal(page->segment_in_use);
  ...
  // note: must come after setting `segment_in_use` to false but before
  // block_size becomes 0
  // mi_page_reset(segment, page, 0 /*used_size*/, tld);

  // zero the page data, but not the segment fields and capacity, and block_size
  // (for page size calculations)
...
void _mi_segment_page_free(mi_page_t* page, bool force,
                           mi_segments_tld_t* tld) {
  mi_assert(page != NULL);
  ...
  if (segment->used == 0) {
    ...
    if (segment->used == segment->abandoned) {
      ...
      mi_assert_internal(
          segment->page_kind <=
          MI_PAGE_MEDIUM);  // for now we only support small and medium pages
...
static mi_tagged_segment_t mi_tagged_segment(mi_segment_t* segment,
...
static mi_decl_cache_align volatile _Atomic(mi_segment_t*)
    abandoned_visited;  // = NULL

// The abandoned page list (tagged as it supports pop)
static mi_decl_cache_align volatile _Atomic(
    mi_tagged_segment_t) abandoned;  // = NULL

// We also maintain a count of current readers of the abandoned list
// in order to prevent resetting/decommitting segment memory if it might
// still be read.
static mi_decl_cache_align volatile _Atomic(uintptr_t)
    abandoned_readers;  // = 0
...
static void mi_abandoned_visited_push(mi_segment_t* segment) {
  ...
  } while (!mi_atomic_cas_ptr_weak(mi_segment_t, &abandoned_visited, segment,
                                   anext));
}
...
static bool mi_abandoned_visited_revisit(void) {
  ...
  if (mi_atomic_read_ptr_relaxed(mi_segment_t, &abandoned_visited) == NULL)
    return false;

  // grab the whole visited list
  mi_segment_t* first =
      mi_atomic_exchange_ptr(mi_segment_t, &abandoned_visited, NULL);
...
static mi_segment_t* mi_abandoned_pop(void) {
  ...
  if (mi_likely(segment == NULL)) {
    if (mi_likely(!mi_abandoned_visited_revisit())) {  // try to swap in the
                                                       // visited list on NULL
      return NULL;
    }
  }
  ...
  mi_atomic_increment(
      &abandoned_readers);  // ensure no segment gets decommitted
  ...
  do {
    ...
    if (segment != NULL) {
      next =
          mi_tagged_segment(segment->abandoned_next,
                            ts);  // note: reads the segment's `abandoned_next`
                                  // field so should not be decommitted
    }
...
static void mi_segment_abandon(mi_segment_t* segment, mi_segments_tld_t* tld) {
  ...
  mi_pages_reset_remove_all_in_segment(
      segment, mi_option_is_enabled(mi_option_abandoned_page_reset), tld);
  mi_segment_remove_from_free_queue(segment, tld);
  mi_assert_internal(segment->next == NULL && segment->prev == NULL);
...
static bool mi_segment_check_free(mi_segment_t* segment, size_t block_size,
  ...
  for (size_t i = 0; i < segment->capacity; i++) {
    ...
    if (page->segment_in_use) {
      ...
      if (mi_page_all_free(page)) {
        ...
        // note: don't clear the page yet as we can only OS reset it once it is
        // reclaimed
        pages_used_empty++;
        has_page = true;
      } else if (page->xblock_size == block_size &&
  ...
  mi_assert_internal(pages_used == segment->used &&
                     pages_used >= pages_used_empty);
...
// set `right_page_reclaimed` to `true` if it reclaimed a page of the right
// `block_size` that was not full.
static mi_segment_t* mi_segment_reclaim(mi_segment_t* segment, mi_heap_t* heap,
  ...
  for (size_t i = 0; i < segment->capacity; i++) {
    ...
    if (page->segment_in_use) {
      ...
      mi_assert_internal(page->is_committed);
      mi_assert_internal(mi_page_not_in_queue(page, tld));
      mi_assert_internal(mi_page_thread_free_flag(page) ==
      ...
      // TODO: should we not collect again given that we just collected in
      // `check_free`?
    ...
    } else if (page->is_committed &&
               !page->is_reset) {  // not in-use, and not reset yet
  ...
  if (segment->used == 0) {
    mi_assert_internal(right_page_reclaimed == NULL ||
...
static mi_segment_t* mi_segment_try_reclaim(mi_heap_t* heap, size_t block_size,
                                            mi_page_kind_t page_kind,
                                            bool* reclaimed,
                                            mi_segments_tld_t* tld) {
  *reclaimed = false;
  ...
  while ((max_tries-- > 0) && ((segment = mi_abandoned_pop()) != NULL)) {
    ...
    bool has_page = mi_segment_check_free(
        segment, block_size,
        &all_pages_free);  // try to free up pages (due to concurrent frees)
    if (all_pages_free) {
      ...
      // reclaiming another segment that is still partially used. note2: we
      // could in principle optimize this by skipping reclaim and directly
      // freeing but that would violate some invariants temporarily)
      mi_segment_reclaim(segment, heap, 0, NULL, tld);
    } else if (has_page && segment->page_kind == page_kind) {
      // found a free page of the right kind, or page of the right block_size
      // with free space we return the result of reclaim (which is usually
      // `segment`) as it might free the segment due to concurrent frees (in
      ...
      // otherwise, push on the visited list so it gets not looked at too
      // quickly again
...
static mi_segment_t* mi_segment_reclaim_or_alloc(
    mi_heap_t* heap, size_t block_size, mi_page_kind_t page_kind,
    size_t page_shift, mi_segments_tld_t* tld, mi_os_tld_t* os_tld) {
  ...
  bool reclaimed;
  segment =
      mi_segment_try_reclaim(heap, block_size, page_kind, &reclaimed, tld);
  if (reclaimed) {
    // reclaimed the right page right into the heap
    mi_assert_internal(segment != NULL && segment->page_kind == page_kind &&
                       page_kind <= MI_PAGE_LARGE);
    return NULL;  // pretend out-of-memory as the page will be in the page queue
                  // of the heap with available blocks
...
static mi_page_t* mi_segment_find_free(mi_segment_t* segment,
  ...
  mi_assert_internal(mi_segment_has_free(segment));
  mi_assert_expensive(mi_segment_is_valid(segment, tld));
  for (size_t i = 0; i < segment->capacity;
       i++) {  // TODO: use a bitmap instead of search?
    mi_page_t* page = &segment->pages[i];
    if (!page->segment_in_use) {
...
static mi_page_t* mi_segment_page_alloc_in(mi_segment_t* segment,
                                           mi_segments_tld_t* tld) {
  mi_assert_internal(mi_segment_has_free(segment));
  return mi_segment_find_free(segment, tld);
}

static mi_page_t* mi_segment_page_alloc(mi_heap_t* heap, size_t block_size,
                                        mi_page_kind_t kind, size_t page_shift,
                                        mi_segments_tld_t* tld,
                                        mi_os_tld_t* os_tld) {
  // find an available segment the segment free queue
  mi_segment_queue_t* const free_queue =
      mi_segment_free_queue_of_kind(kind, tld);
  if (mi_segment_queue_is_empty(free_queue)) {
    // possibly allocate or reclaim a fresh segment
    mi_segment_t* const segment = mi_segment_reclaim_or_alloc(
        heap, block_size, kind, page_shift, tld, os_tld);
    if (segment == NULL)
      return NULL;  // return NULL if out-of-memory (or reclaimed)
  ...
  _mi_segment_page_start(_mi_page_segment(page), page, sizeof(void*), NULL,
                         NULL)[0] = 0;
#endif
  return page;
}

static mi_page_t* mi_segment_small_page_alloc(mi_heap_t* heap,
                                              size_t block_size,
                                              mi_segments_tld_t* tld,
                                              mi_os_tld_t* os_tld) {
  return mi_segment_page_alloc(heap, block_size, MI_PAGE_SMALL,
                               MI_SMALL_PAGE_SHIFT, tld, os_tld);
}

static mi_page_t* mi_segment_medium_page_alloc(mi_heap_t* heap,
                                               size_t block_size,
                                               mi_segments_tld_t* tld,
                                               mi_os_tld_t* os_tld) {
  return mi_segment_page_alloc(heap, block_size, MI_PAGE_MEDIUM,
                               MI_MEDIUM_PAGE_SHIFT, tld, os_tld);
}
...
static mi_page_t* mi_segment_large_page_alloc(mi_heap_t* heap,
                                              size_t block_size,
                                              mi_segments_tld_t* tld,
                                              mi_os_tld_t* os_tld) {
  mi_segment_t* segment = mi_segment_reclaim_or_alloc(
      heap, block_size, MI_PAGE_LARGE, MI_LARGE_PAGE_SHIFT, tld, os_tld);
  if (segment == NULL) return NULL;
...
static mi_page_t* mi_segment_huge_page_alloc(size_t size,
                                             mi_segments_tld_t* tld,
                                             mi_os_tld_t* os_tld) {
  mi_segment_t* segment =
      mi_segment_alloc(size, MI_PAGE_HUGE, MI_SEGMENT_SHIFT, tld, os_tld);
  if (segment == NULL) return NULL;
  mi_assert_internal(mi_segment_page_size(segment) -
                         segment->segment_info_size -
                         (2 * (MI_SECURE == 0 ? 0 : _mi_os_page_size())) >=
                     size);
...
void _mi_segment_huge_page_free(mi_segment_t* segment, mi_page_t* page,
  ...
  // claim it and free
  mi_heap_t* heap =
      mi_heap_get_default();  // issue #221; don't use the internal
                              // get_default_heap as we need to ensure the
...
mi_page_t* _mi_segment_page_alloc(mi_heap_t* heap, size_t block_size,
  ...
  mi_assert_internal(page == NULL ||
                     (mi_segment_page_size(_mi_page_segment(page)) -
                      (MI_SECURE == 0 ? 0 : _mi_os_page_size())) >= block_size);
  mi_reset_delayed(tld);
