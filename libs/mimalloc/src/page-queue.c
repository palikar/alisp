...
static inline uint8_t mi_bsr32(uint32_t x) { return (31 - __builtin_clz(x)); }
#else
static inline uint8_t mi_bsr32(uint32_t x) {
...
#if (MI_DEBUG > 1)
static bool mi_page_queue_contains(mi_page_queue_t* queue,
...
#if (MI_DEBUG > 1)
static bool mi_heap_contains_queue(const mi_heap_t* heap,
...
static mi_page_queue_t* mi_page_queue_of(const mi_page_t* page) {
  uint8_t bin =
      (mi_page_is_in_full(page) ? MI_BIN_FULL : _mi_bin(page->xblock_size));
...
static mi_page_queue_t* mi_heap_page_queue_of(mi_heap_t* heap,
                                              const mi_page_t* page) {
  uint8_t bin =
      (mi_page_is_in_full(page) ? MI_BIN_FULL : _mi_bin(page->xblock_size));
  ...
  mi_assert_internal(mi_page_is_in_full(page) ||
                     page->xblock_size == pq->block_size);
  return pq;
}
...
static inline void mi_heap_queue_first_update(mi_heap_t* heap,
                                              const mi_page_queue_t* pq) {
  ...
  if (idx <= 1) {
    ...
    // find previous size; due to minimal alignment upto 3 previous bins may
    // need to be skipped
...
static void mi_page_queue_remove(mi_page_queue_t* queue, mi_page_t* page) {
  ...
  mi_assert_internal(
      ...
      (page->xblock_size > MI_LARGE_OBJ_SIZE_MAX &&
       mi_page_queue_is_huge(queue)) ||
...
static void mi_page_queue_push(mi_heap_t* heap, mi_page_queue_t* queue,
                               mi_page_t* page) {
...
static void mi_page_queue_enqueue_from(mi_page_queue_t* to,
                                       mi_page_queue_t* from, mi_page_t* page) {
  mi_assert_internal(page != NULL);
  mi_assert_expensive(mi_page_queue_contains(from, page));
  mi_assert_expensive(!mi_page_queue_contains(to, page));
  mi_assert_internal(
      (page->xblock_size == to->block_size &&
       page->xblock_size == from->block_size) ||
      (page->xblock_size == to->block_size && mi_page_queue_is_full(from)) ||
      (page->xblock_size == from->block_size && mi_page_queue_is_full(to)) ||
      (page->xblock_size > MI_LARGE_OBJ_SIZE_MAX &&
       mi_page_queue_is_huge(to)) ||
...
size_t _mi_page_queue_append(mi_heap_t* heap, mi_page_queue_t* pq,
  ...
  for (mi_page_t* page = append->first; page != NULL; page = page->next) {
    ...
    // set the flag to delayed free (not overriding NEVER_DELAYED_FREE) which
    // has as a side effect that it spins until any DELAYED_FREEING is finished.
    // This ensures that after appending only the new heap will be used for
    // delayed free operations.
