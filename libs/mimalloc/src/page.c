...
static void mi_page_init(mi_heap_t* heap, mi_page_t* page, size_t size,
                         mi_tld_t* tld);
...
static bool mi_page_is_valid_init(mi_page_t* page) {
  ...
  mi_assert_internal(start ==
                     _mi_segment_page_start(segment, page, bsize, NULL, NULL));
  ...
  if (page->flags.is_zero) {
    for (mi_block_t* block = page->free; block != NULL;
      ...
      mi_assert_expensive(
          mi_mem_is_zero(block + 1, page->block_size - sizeof(mi_block_t)));
    }
  }
#endif
  ...
  if (mi_page_heap(page) != NULL) {
    ...
    if (segment->page_kind != MI_PAGE_HUGE) {
      ...
      mi_assert_internal(pq->block_size == mi_page_block_size(page) ||
                         mi_page_block_size(page) > MI_LARGE_OBJ_SIZE_MAX ||
...
void _mi_page_use_delayed_free(mi_page_t* page, mi_delayed_t delay,
  ...
  do {
    tfree = mi_atomic_read(
        &page->xthread_free);  // note: must acquire as we can break this loop
                               // and not do a CAS
    tfreex = mi_tf_set_delayed(tfree, delay);
    old_delay = mi_tf_delayed(tfree);
    if (mi_unlikely(old_delay == MI_DELAYED_FREEING)) {
      mi_atomic_yield();  // delay until outstanding MI_DELAYED_FREEING are
                          // done.
...
// ensure that there was no race where the page became unfull just before the
// move.
...
void _mi_page_free_collect(mi_page_t* page, bool force) {
  ...
  if (force || mi_page_thread_free(page) !=
                   NULL) {  // quick test to avoid an atomic operation
...
static mi_page_t* mi_page_fresh_alloc(mi_heap_t* heap, mi_page_queue_t* pq,
                                      size_t block_size) {
  ...
  mi_page_t* page = _mi_segment_page_alloc(
      heap, block_size, &heap->tld->segments, &heap->tld->os);
  if (page == NULL) {
  ...
  mi_assert_internal(pq == NULL ||
                     _mi_page_segment(page)->page_kind != MI_PAGE_HUGE);
...
void _mi_heap_delayed_free(mi_heap_t* heap) {
  ...
  do {
    block = mi_atomic_read_ptr_relaxed(mi_block_t, &heap->thread_delayed_free);
           ...
           !mi_atomic_cas_ptr_weak(mi_block_t, &heap->thread_delayed_free, NULL,
                                   block));
...
static void mi_page_to_full(mi_page_t* page, mi_page_queue_t* pq) {
  ...
  _mi_page_free_collect(
      page, false);  // try to collect right away in case another thread freed
...
void _mi_page_abandon(mi_page_t* page, mi_page_queue_t* pq) {
  ...
  for (mi_block_t* block = (mi_block_t*)pheap->thread_delayed_free;
       block != NULL; block = mi_block_nextx(pheap, block, pheap->keys)) {
...
void _mi_heap_collect_retired(mi_heap_t* heap, bool force) {
  ...
  size_t max = 0;
  for (size_t bin = heap->page_retired_min; bin <= heap->page_retired_max;
       bin++) {
...
static void mi_page_free_list_extend_secure(mi_heap_t* const heap,
  ...
  for (size_t i = 0; i < slice_count; i++) {
    blocks[i] = mi_page_block_at(page, page_area, bsize,
                                 page->capacity + i * slice_extend);
    counts[i] = slice_extend;
  }
  counts[slice_count - 1] +=
      (extend % slice_count);  // final slice holds the modulus too (todo:
                               // distribute evenly?)
...
static mi_decl_noinline void mi_page_free_list_extend(mi_page_t* const page,
  ...
  mi_block_t* const start =
      mi_page_block_at(page, page_area, bsize, page->capacity);
...
#define MI_MAX_EXTEND_SIZE \
  (4 * 1024)  // heuristic, one OS page seems to work well.
#if (MI_SECURE > 0)
...
static void mi_page_init(mi_heap_t* heap, mi_page_t* page, size_t block_size,
                         mi_tld_t* tld) {
  mi_assert(page != NULL);
  mi_segment_t* segment = _mi_page_segment(page);
  ...
  page->xblock_size = (block_size < MI_HUGE_BLOCK_SIZE ? (uint32_t)block_size
                                                       : MI_HUGE_BLOCK_SIZE);
...
static mi_page_t* mi_page_queue_find_free_ex(mi_heap_t* heap,
                                             mi_page_queue_t* pq,
                                             bool first_try) {
  ...
  if (page == NULL) {
    ...
    if (page == NULL && first_try) {
      // out-of-memory _or_ an abandoned page with free blocks was reclaimed,
      // try once again
...
static inline mi_page_t* mi_find_free_page(mi_heap_t* heap, size_t size) {
  ...
  mi_page_t* page = pq->first;
  if (page != NULL) {
    if ((MI_SECURE >= 3) && page->capacity < page->reserved &&
...
void _mi_deferred_free(mi_heap_t* heap, bool force) {
  ...
  if (deferred_free != NULL && !heap->tld->recurse) {
    ...
    deferred_free(force, heap->tld->heartbeat,
                  mi_atomic_read_ptr_relaxed(void, &deferred_arg));
    heap->tld->recurse = false;
  }
}

void mi_register_deferred_free(mi_deferred_free_fun* fn,
...
static mi_page_t* mi_huge_page_alloc(mi_heap_t* heap, size_t size) {
  ...
  if (page != NULL) {
    const size_t bsize =
        mi_page_block_size(page);  // note: not `mi_page_usable_block_size` as
                                   // `size` includes padding already
    mi_assert_internal(bsize >= size);
    mi_assert_internal(mi_page_immediate_available(page));
    mi_assert_internal(_mi_page_segment(page)->page_kind == MI_PAGE_HUGE);
    mi_assert_internal(_mi_page_segment(page)->used == 1);
    mi_assert_internal(_mi_page_segment(page)->thread_id ==
                       0);  // abandoned, not in the huge queue
    mi_page_set_heap(page, NULL);
...
// Allocate a page
// Note: in debug mode the size includes MI_PADDING_SIZE and might have
// overflowed.
static mi_page_t* mi_find_page(mi_heap_t* heap, size_t size) mi_attr_noexcept {
  // huge allocation?
  const size_t req_size =
      size - MI_PADDING_SIZE;  // correct for padding_size in case of an
                               // overflow on `size`
  if (mi_unlikely(req_size > (MI_LARGE_OBJ_SIZE_MAX - MI_PADDING_SIZE))) {
    if (mi_unlikely(
      ...
      _mi_error_message(EOVERFLOW,
                        "allocation request is too large (%zu b requested)\n",
                        req_size);
      return NULL;
    } else {
...
// Generic allocation routine if the fast path (`alloc.c:mi_page_malloc`) does
// not succeed. Note: in debug mode the size includes MI_PADDING_SIZE and might
// have overflowed.
void* _mi_malloc_generic(mi_heap_t* heap, size_t size) mi_attr_noexcept {
  ...
  if (mi_unlikely(page == NULL)) {  // first time out of memory, try to collect
                                    // and retry the allocation once more
