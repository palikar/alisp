...
In contrast to the rest of mimalloc, the "regions" are shared between threads
and need to be accessed using atomic operations. We need this memory layer
between the raw OS calls because of:
1. on `sbrk` like systems (like WebAssembly) we need our own memory maps in
order to reuse memory effectively.
...
Possible issues:
- (2) can potentially be addressed too with a small cache per thread which is
much simpler. Generally though that requires shrinking of huge pages, and may
overuse memory per thread. (and is not compatible with `sbrk`).
...
// arena.c
void _mi_arena_free(void* p, size_t size, size_t memid, bool all_committed,
                    mi_stats_t* stats);
void* _mi_arena_alloc(size_t size, bool* commit, bool* large, bool* is_zero,
                      size_t* memid, mi_os_tld_t* tld);
void* _mi_arena_alloc_aligned(size_t size, size_t alignment, bool* commit,
                              bool* large, bool* is_zero, size_t* memid,
                              mi_os_tld_t* tld);
...
#define MI_REGION_SIZE \
  (MI_SEGMENT_SIZE * MI_BITMAP_FIELD_BITS)  // 256MiB  (64MiB on 32 bits)
#define MI_REGION_MAX \
  (MI_HEAP_REGION_MAX_SIZE / MI_REGION_SIZE)  // 1024  (48 on 32 bits)
...
typedef union mi_region_info_u {
  ...
  struct {
    ...
    short numa_node;  // the associated NUMA node (where -1 means no associated
                      // node)
  } x;
} mi_region_info_t;
...
typedef struct mem_region_s {
  ...
  volatile _Atomic(uintptr_t)
      arena_memid;    // if allocated from a (huge page) arena
...
/*
// Return a rounded commit/reset size such that we don't fragment large OS pages
into small ones. static size_t mi_good_commit_size(size_t size) { if (size >
(SIZE_MAX - _mi_os_large_page_size())) return size; return _mi_align_up(size,
...
bool mi_is_in_heap_region(const void* p) mi_attr_noexcept {
  ...
  for (size_t i = 0; i < count; i++) {
    ...
    if (start != NULL && (uint8_t*)p >= start &&
        (uint8_t*)p < start + MI_REGION_SIZE)
      return true;
  }
  return false;
}

static void* mi_region_blocks_start(const mem_region_t* region,
...
static bool mi_memid_is_arena(size_t id, mem_region_t** region,
                              mi_bitmap_index_t* bit_idx, size_t* arena_memid) {
  if ((id & 1) == 1) {
...
static bool mi_region_try_alloc_os(size_t blocks, bool commit, bool allow_large,
                                   mem_region_t** region,
                                   mi_bitmap_index_t* bit_idx,
                                   mi_os_tld_t* tld) {
  ...
  bool region_commit =
      (commit && mi_option_is_enabled(mi_option_eager_region_commit));
  bool region_large = (commit && allow_large);
  bool is_zero = false;
  size_t arena_memid = 0;
  void* const start =
      _mi_arena_alloc_aligned(MI_REGION_SIZE, MI_SEGMENT_ALIGN, &region_commit,
                              &region_large, &is_zero, &arena_memid, tld);
  ...
  if (idx >= MI_REGION_MAX) {
    ...
    _mi_arena_free(start, MI_REGION_SIZE, arena_memid, region_commit,
                   tld->stats);
    _mi_warning_message(
        "maximum regions used: %zu GiB (perhaps recompile with a larger "
        "setting for MI_HEAP_REGION_MAX_SIZE)",
...
static bool mi_region_is_suitable(const mem_region_t* region, int numa_node,
                                  bool allow_large) {
  // initialized at all?
  mi_region_info_t info;
...
static bool mi_region_try_claim(int numa_node, size_t blocks, bool allow_large,
                                mem_region_t** region,
                                mi_bitmap_index_t* bit_idx, mi_os_tld_t* tld) {
  // try all regions for a free slot
  const size_t count = mi_atomic_read(&regions_count);
  size_t idx =
      tld->region_idx;  // Or start at 0 to reuse low addresses? Starting at 0
                        // seems to increase latency though
  for (size_t visited = 0; visited < count; visited++, idx++) {
    if (idx >= count) idx = 0;  // wrap around
    mem_region_t* r = &regions[idx];
    // if this region suits our demand (numa node matches, large OS page
    // matches)
...
static void* mi_region_try_alloc(size_t blocks, bool* commit, bool* is_large,
                                 bool* is_zero, size_t* memid,
                                 mi_os_tld_t* tld) {
  mi_assert_internal(blocks <= MI_BITMAP_FIELD_BITS);
  mem_region_t* region;
  mi_bitmap_index_t bit_idx;
  const int numa_node =
      (_mi_os_numa_node_count() <= 1 ? -1 : _mi_os_numa_node(tld));
  // try to claim in existing regions
  if (!mi_region_try_claim(numa_node, blocks, *is_large, &region, &bit_idx,
                           tld)) {
    // otherwise try to allocate a fresh region and claim in there
    if (!mi_region_try_alloc_os(blocks, *commit, *is_large, &region, &bit_idx,
                                tld)) {
  ...
  *memid = mi_memid_create(region, bit_idx);
  ...
  mi_assert_internal(!*commit ||
                     mi_bitmap_is_claimed(&region->commit, 1, blocks, bit_idx));

  // unreset reset blocks
  if (mi_bitmap_is_any_claimed(&region->reset, 1, blocks, bit_idx)) {
    // some blocks are still reset
    mi_assert_internal(!info.x.is_large);
    mi_assert_internal(!mi_option_is_enabled(mi_option_eager_commit) ||
                       *commit ||
                       mi_option_get(mi_option_eager_commit_delay) > 0);
    mi_bitmap_unclaim(&region->reset, 1, blocks, bit_idx);
    if (*commit ||
        !mi_option_is_enabled(mi_option_reset_decommits)) {  // only if needed
  ...
  mi_assert_internal(
      !mi_bitmap_is_any_claimed(&region->reset, 1, blocks, bit_idx));
...
// Allocate `size` memory aligned at `alignment`. Return non NULL on success,
// with a given memory `id`.
// (`id` is abstract, but `id = idx*MI_REGION_MAP_BITS + bitidx`)
void* _mi_mem_alloc_aligned(size_t size, size_t alignment, bool* commit,
                            bool* large, bool* is_zero, size_t* memid,
                            mi_os_tld_t* tld) {
  ...
  *is_zero = false;
  ...
  if (p == NULL) {
    ...
    p = _mi_arena_alloc_aligned(size, alignment, commit, large, is_zero,
                                &arena_memid, tld);
...
void _mi_mem_free(void* p, size_t size, size_t id, bool full_commit,
                  bool any_reset, mi_os_tld_t* tld) {
  ...
  if (mi_memid_is_arena(id, &region, &bit_idx, &arena_memid)) {
    ...
    mi_assert_internal(size <= MI_REGION_MAX_OBJ_SIZE);
    if (size > MI_REGION_MAX_OBJ_SIZE) return;
    const size_t blocks = mi_region_block_count(size);
    ...
    if (!info.x.is_large && mi_option_is_enabled(mi_option_segment_reset) &&
        (mi_option_is_enabled(mi_option_eager_commit) ||
         mi_option_is_enabled(
             mi_option_reset_decommits)))  // cannot reset halfway committed
                                           // segments, use only
                                           // `option_page_reset` instead
    {
      bool any_unreset;
      mi_bitmap_claim(&region->reset, 1, blocks, bit_idx, &any_unreset);
      if (any_unreset) {
        _mi_abandoned_await_readers();  // ensure no more pending write (in case
                                        // reset = decommit)
    ...
    if (mi_atomic_read_relaxed(&region->info) != 0) {
      ...
      do {
        m = mi_atomic_read_relaxed(&region->in_use);
               ...
               !mi_atomic_cas_weak(&region->in_use, MI_BITMAP_FIELD_FULL, 0));
      if (m == 0) {
        ...
        if (start != NULL) {  // && !_mi_os_is_huge_reserved(start)) {
          ...
          _mi_arena_free(start, MI_REGION_SIZE, arena_memid, (~commit == 0),
                         tld->stats);
        }
      }
    }
