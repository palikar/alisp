...
ALObjectPtr Fget_env(const ALObjectPtr &t_obj, env::Environment *, eval::Evaluator *eval)
{
...
ALObjectPtr Fcheck_env(const ALObjectPtr &t_obj, env::Environment *, eval::Evaluator *eval)
{
    AL_CHECK(assert_size<1>(t_obj));
    auto var = eval->eval(t_obj->i(0));
    AL_CHECK(assert_string(var));
...
ALObjectPtr Fset_env(const ALObjectPtr &t_obj, env::Environment *, eval::Evaluator *eval)
{
...
ALObjectPtr Flist_env(const ALObjectPtr &t_obj, env::Environment *, eval::Evaluator *)
{
    AL_CHECK(assert_size<0>(t_obj));
    return get_evnvars();
}

ALObjectPtr Fchwd(const ALObjectPtr &t_obj, env::Environment *, eval::Evaluator *eval)
{
...
ALObjectPtr Fsystem(const ALObjectPtr &t_obj, env::Environment *, eval::Evaluator *eval)
{
    AL_CHECK(assert_size<1>(t_obj));
