...
template<class Environment> class ALParser : public ParserBase
{
  ...
  private:
    ...
    template<typename number_type, int m_radix> struct WholeNumberParser
    {
        ...
        number_type get_num() { return m_num * sign; }
    };

    template<typename number_type> struct RealNumberParser
    {
        number_type m_num           = 0;
    ...
    template<int base> ALObjectPtr parse_integer()
    {
