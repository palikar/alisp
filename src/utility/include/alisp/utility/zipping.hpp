...
template<typename... Iterators> void advance_all(Iterators &... iterators)
{
    (std::next(iterators), ...);
}
...
template<typename InType,
         ...
         class InContainer,
         template<typename V, typename alloc = std::allocator<V>> class OutContainer = InContainer,
         typename OutType                                                            = InType>
OutContainer<OutType> mapf(const InContainer<InType> &input, std::function<OutType(const InType &)> func)
{
