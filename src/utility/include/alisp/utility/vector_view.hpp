...
template<typename T> class vector_view
{
  ...
  public:
    ...
    const reference operator[](size_t index) const { return *std::next(m_first, static_cast<difference_type>(index)); }

    size_type size() const { return static_cast<size_type>(std::distance(m_first, m_last)); }
...
template<typename T> vector_view(std::vector<T> &t_vec)->vector_view<T>;
