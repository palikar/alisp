...
namespace nu::logging
{
    ...
    const std::string file_name{"./numler_log.txt"};
...
#if DEBUG_LOGGING
#define DEBUG(...) nu::logging::_dev_debug->debug(__VA_ARGS__)
#else
#define DEBUG(...) (void)0
#endif
...
#define ABORT(...) \
    do {           \
        nu::logging::_main_logger->critical(__VA_ARGS__);       \
        nu::logging::_main_logger->critical("Aborting!");       \
        abort();                                                \
    } while (0)
